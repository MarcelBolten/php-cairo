...
PHP_MINIT_FUNCTION(cairo_exception)
{
	...
	CAIRO_STATUS_DECLARE_ENUM(WIN32_GDI_ERROR);
	CAIRO_STATUS_DECLARE_ENUM(FREETYPE_ERROR);
	CAIRO_STATUS_DECLARE_ENUM(PNG_ERROR);
	CAIRO_STATUS_DECLARE_ENUM(TAG_ERROR);
	#endif

	#if CAIRO_VERSION >= CAIRO_VERSION_ENCODE(1, 18, 0)
	CAIRO_STATUS_DECLARE_ENUM(DWRITE_ERROR);
	CAIRO_STATUS_DECLARE_ENUM(SVG_FONT_ERROR);
	#endif
