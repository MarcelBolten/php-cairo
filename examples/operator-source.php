...
function setGradientPattern($context, $width, $height, $x, $y)
{
...
function setSurfacePattern($surface, $context, $width, $height, $x, $y)
{
	$wi = floor(0.6 * $width);
	$he = floor(0.6 * $height);
	$x += 0.2 * $width;
	...
	for ($i = 0; $i < 4; $i++)
	{
		...
		switch ($i)
		{
			case 0:
				...
				break;
			case 1:
				setTranslucentPattern($context, $x, $y);
				break;
			case 2:
...
$surface->writeToPng(dirname(__FILE__).'/operator-source-php.png');
