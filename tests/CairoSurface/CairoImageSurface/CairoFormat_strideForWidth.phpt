...
/* Wrong number args 2 */
try {
    Cairo\Surface\ImageFormat::strideForWidth(Cairo\Surface\ImageFormat::from(1));
} catch (TypeError $e) {
    echo $e->getMessage(), PHP_EOL;
}
...
/* Wrong number args 3 */
try {
    Cairo\Surface\ImageFormat::strideForWidth(Cairo\Surface\ImageFormat::from(1), 1, 1);
} catch (TypeError $e) {
    echo $e->getMessage(), PHP_EOL;
}
...
/* Wrong arg type */
try {
    Cairo\Surface\ImageFormat::strideForWidth(Cairo\Surface\ImageFormat::from(1), array());
} catch (TypeError $e) {
    echo $e->getMessage(), PHP_EOL;
}
--EXPECTF--
...
Cairo\Surface\ImageFormat::strideForWidth(): Argument #1 ($format) must be of type Cairo\Surface\ImageFormat, array given
